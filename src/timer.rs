...
pub trait WithTimeout {
    ...
    /// If the future completes before the timeout, its output is returned. Otherwise, on timeout,
    /// work on the future is stopped (`poll` is no longer called), the future is dropped and `Err(TimeoutError)` is returned.
    fn with_timeout(self, timeout: Duration) -> impl Future<Output = Result<Self::Output, TimeoutError>>;

    /// Runs a given future with a deadline time.
    ///
    /// If the future completes before the deadline, its output is returned. Otherwise, on timeout,
    /// work on the future is stopped (`poll` is no longer called), the future is dropped and `Err(TimeoutError)` is returned.
    fn with_deadline(self, at: Instant) -> impl Future<Output = Result<Self::Output, TimeoutError>>;
}
...
impl Future for Timer {
    ...
    fn poll(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        if self.yielded_once && Instant::now() >= self.expires_at {
            Poll::Ready(())
        } else {
            // TODO: we should probably replace this and other wakers by setting an interrupt in the
            //       time driver to fire when sufficiently many ticks have passed
...
impl Ticker {
    ...
    pub fn tick(&mut self) -> impl Future<Output = ()> + Send + Sync + '_ {
        poll_fn(|cx| {
